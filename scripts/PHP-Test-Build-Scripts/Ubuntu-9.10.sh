...
if [ -d $php52_src_dir ]; then
  if [ $force_state = "yes" ]; then
    ...
    rm -rf "./${php52_src_dir}/*"
    if [ "$?" -ne 0 ]; then echo "Failed while cleaning the \"${php52_src_dir}\" directory."; fi
  else
  ...
  mkdir $php52_src_dir
  if [ "$?" -ne 0 ]; then echo "Failed while creating the ${php52_src_dir} directory."; fi
fi
...
if [ -d $php53_src_dir ]; then
  if [ $force_state = "yes" ]; then
    ...
    rm -rf "./${php53_src_dir}/*"
    if [ "$?" -ne 0 ]; then echo "Failed while cleaning the \"${php53_src_dir}\" directory."; fi
  else
  ...
  mkdir $php53_src_dir
  if [ "$?" -ne 0 ]; then echo "Failed while creating the ${php53_src_dir} directory."; fi
fi
...
if [ -f $php52_archive ]; then
  if [ $cached_state = "no" ]; then
    rm -f $php52_archive
    if [ "$?" -ne 0 ]; then echo "Failed removing old archive ${php52_archive}."; fi
  fi
fi

if [ -f $php52_archive -o ! $build_state = "all" -a ! $build_state = "52" ]; then
  echo "Skipping download of ${php52_archive}."
else 
  echo "Downloading ${php52_archive} from snaps.php.net."
  wget -O $php52_archive $php52_snaps
...
if [ -f $php53_archive ]; then
  if [ $cached_state = "no" ]; then
    rm -f $php53_archive
    if [ "$?" -ne 0 ]; then echo "Failed removing old archive ${php53_archive}."; fi
  fi
fi

if [ -f $php53_archive -o ! $build_state = "all" -a ! $build_state = "53" ]; then
  echo "Skipping download of ${php53_archive}."
else 
  echo "Downloading ${php53_archive} from snaps.php.net."
  wget -O $php53_archive $php53_snaps
...
if [ "$?" -ne 0 ]; then echo "Failed extracting ${php52_archive}."; fi

echo "Extracting ${php53_archive} source."
tar -xzf $php53_archive --strip=1 -C "./${php53_src_dir}"
if [ "$?" -ne 0 ]; then echo "Failed extracting ${php53_archive}."; fi
...
cd $php52_src_dir
if [ "$?" -ne 0 ]; then echo "Failed to change directory to ${php52_src_dir}."; fi
...
if [ "$?" -ne 0 ]; then echo "Failed to configure the PHP 5.2 source."; fi

echo "Building the PHP 5.2 binary."
make
if [ "$?" -ne 0 ]; then echo "Failed to build the PHP 5.2 binary."; fi

cd "../${php53_src_dir}"
if [ "$?" -ne 0 ]; then echo "Failed to change directory to ${php53_src_dir}."; fi
...
if [ "$?" -ne 0 ]; then echo "Failed to configure the PHP 5.3 source."; fi

echo "Building the PHP 5.3 binary."
make
if [ "$?" -ne 0 ]; then echo "Failed to build the PHP 5.3 binary."; fi
...
if [ $cached_state = "no" ]; then
  ...
  rm -f $php52_archive
  if [ "$?" -ne 0 ]; then echo "Failed to remove ${php52_archive}."; fi
  
  echo "Cleaning up ${php53_archive}."
  rm -f $php53_archive
  if [ "$?" -ne 0 ]; then echo "Failed to remove ${php53_archive}."; fi
